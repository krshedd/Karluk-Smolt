...
# lnvar=log(CVvec[mix]^2+1)
# lnmean=log(catchvec[mix])-lnvar/2
# rlnorm(nsamps[chain,mix],lnmean,sqrt(lnvar))
# 
# 
# 
# for(boot in seq(boots)) {
#   
#   for(day in days) {
#     dirich = rdirich(alpha = dat[day, c("Age1", "Age2", "Age3", "Age4")])
#     
#     lnvar=log(dat$CV[day]^2 + 1)
#     lnmean=log(dat$DailyOM[day])-lnvar/2
#     
#     N = rlnorm(n = 1, meanlog = lnmean, sdlog = sqrt(lnvar))
#     
#     Nage = N * dirich
#   }
# }
...
# Testing for first SILLY
KarlukSmolt2013.1_Metadata <- Metadata[match(KarlukSmolt2013.1.gcl$attributes$SillySource, Metadata$SILLY),]
dim(KarlukSmolt2013.1_Metadata)

KarlukSmolt2013.1_IA <- lapply(KarlukSmolt_IndividualAssignments, function(silly) {list("Early"=which(silly[,1]>=0.9), "Late"=which(silly[,2]>=0.9))})[[1]]

x <- rep(NA, length=dim(KarlukSmolt2013.1_Metadata)[1])

x[KarlukSmolt2013.1_IA[[1]]] <- "Early"
x[KarlukSmolt2013.1_IA[[2]]] <- "Late"

KarlukSmolt2013.1_Metadata$RG <- as.factor(x)

by(data=KarlukSmolt2013.1_Metadata$AGE, INDICES=KarlukSmolt2013.1_Metadata$RG, FUN=table)
by(data=KarlukSmolt2013.1_Metadata$AGE, INDICES=KarlukSmolt2013.1_Metadata$RG, FUN=sum)

by(data=KarlukSmolt2013.1_Metadata$FL, INDICES=KarlukSmolt2013.1_Metadata$RG, FUN=table)
by(data=KarlukSmolt2013.1_Metadata$K, INDICES=KarlukSmolt2013.1_Metadata$RG, FUN=table)


boxplot(FL ~ RG*AGE, data=KarlukSmolt2013.1_Metadata, notch=FALSE, col=c("blue", "red")) # use this
boxplot(WEIGHT ~ RG*AGE, data=KarlukSmolt2013.1_Metadata, notch=FALSE, col=c("blue", "red")) # use this
boxplot(K ~ RG*AGE, data=KarlukSmolt2013.1_Metadata, notch=FALSE, col=c("blue", "red")) # use this
...
# Metadata inputs
Metadata <- read.table(file="KarlukMetadata2013.csv", as.is=TRUE, header=TRUE, sep=",")
str(Metadata)
...
mixnames <- KarlukMixtures # mixnames <- paste("KarlukEarly_",RGprop,"_1",sep='') # CAN ONLY USE 2013 MIXTURES WITH THE 2013 METADATA FILE
mixsampsizes <- KarlukMixtures_SampleSizes[,"Final"] # mixsampsizes <- 200
BAYESoutputDir <- paste(getwd(),"/BAYES/Output/",KarlukMixtures,sep='') # BAYESoutputDir <- "V:/WORK/Sockeye/Kodiak/2014 Karluk Baseline/BAYES/BAYES.output/IndividualAssignment/"
nchains <- 5
nreps <- 40000
burn <- 0.5
thin <- 100

metadata=c("FL","Weight","K","pAge0","pAge1","pAge2","pAge3","pAge4")

multi.fun <- function(x) {c("mean"=mean(x),"sd"=sd(x),"n"=length(x))}



names(mixnames)=names(mixsampsizes)=mixnames
...
for(mixname in mixnames){
  ...
  skip <- mixsampsizes[[mixname]]*floor(nreps[[mixname]]*burn/thin)
  
  nits <- floor(nreps[[mixname]]*burn/thin*nchains[[mixname]])
  
  output[[mixname]] <- NULL
  
  results[[mixname]] <- array(data=NA,dim=c(nits,max(groupvec),length(metadata)),dimnames=list(seq(nits),GroupNames,metadata))
  
  for(chain in chains){ #maybe come back and build shell and fill in, as opposed to growing w/ rbind # also maybe switch to scan instead of read.table and use a matrix instead of a dataframe
    
    output[[mixname]] <- rbind(output[[mixname]],read.table(myfilenames[[mixname]][match(chain,chains)],header=FALSE,skip=skip))
    
  }#chain 
  
  # Get individual assignments to RG for every iteration
  
  output_RG <- groupvec[apply(output[[mixname]],1,which.max)]
  
  # Determine metadata for each individual
  # This will be the same for FL, Weight, and K
  # Needs to be done BY AGE
  
  output_FL <- Metadata$FL[match(as.character(get(paste(mixname,".gcl",sep=''))$attributes$SillySource),Metadata$SILLY)] # FL's for each mixture individual
  # output_FL <- c(rnorm(n=RGprop*mixsampsizes[[mixname]],mean=100,sd=10),rnorm(n=(1-RGprop)*mixsampsizes[[mixname]],mean=120,sd=10)) # FAKE DATA
  
  ## DO WE NEED TO DO THIS FOR EACH AGE CLASS???? YES!!!!!!  
  
  output_FL_summary <- tapply(output_RG,rep(x=seq(nits),each=mixsampsizes[[mixname]]),function(zz){matrix(data=c(multi.fun(output_FL[which(zz==1)]),multi.fun(output_FL[which(zz==2)])),nrow=3,ncol=max(groupvec),dimnames=list(c("Mean","SD","n"),GroupNames))}) # determin the mean, sd, and n for FL for each RG at each iteration
  
  output_FL_results <- lapply(output_FL_summary,function(iter){sapply(GroupNames,function(RG){rnorm(n=1,mean=iter["Mean",RG],sd=iter["SD",RG]/sqrt(iter["n",RG]))})}) # account for sampling error by randomly drawing a mean population FL from rnorm using the mean and SE of FLs from each RG
  
  hist(unlist(lapply(output_FL_results,function(lst) lst[1])),col=4,xlim=c(95,125))
  hist(unlist(lapply(output_FL_results,function(lst) lst[2])),col=2,add=TRUE)
  abline(v=c(100,120),lwd=8,lty=3)
  
  
  
  
  #### Compare the above to hard assignments based on a cutoff ####
  source(file="V:/DATA/R_GEN/tempGCL Source Scripts/IndividualAssignmentSummaryKS.GCL.R")
  
  KarlukEarlyIndividualAssignments <-
    IndividualAssignmentSummaryKS.GCL(GroupNames=GroupNames,groupvec=groupvec,mixnames=mixnames,
                                      BAYESoutputDir="V:/WORK/Sockeye/Kodiak/2014 Karluk Baseline/BAYES/BAYES.output/IndividualAssignment",nchains=5,nreps=40000,
                                      burn=1/2,thin=100)
  
  assignments <- cbind(KarlukEarlyIndividualAssignments[[1]],output_FL)
  
  cutoff <- 0.8
  
  summary(assignments[which(assignments[,1]>cutoff),3]); length(assignments[which(assignments[,1]>cutoff),3])
  summary(assignments[which(assignments[,2]>cutoff),3]); length(assignments[which(assignments[,2]>cutoff),3])
  
  
  hist(assignments[which(assignments[,1]>cutoff),3], col=4, xlim=c(70,150), ylim=c(0,30))
  hist(assignments[which(assignments[,2]>cutoff),3], col=2, add=TRUE)
  
  t.test(x=assignments[which(assignments[,1]>cutoff),3],
         y=assignments[which(assignments[,2]>cutoff),3])
  ...
  output_Age <- Metadata$AGE[match(as.character(get(paste(mixname,".gcl",sep=''))$attributes$SillySource),Metadata$SILLY)] # FL's for each mixture individual
  # output_Age <- c(sample(x=0:4,size=100,replace=TRUE,prob=c(0.05,0.25,0.69,0.009,0.001)), sample(x=0:4,size=100,replace=TRUE,prob=c(0.10,0.40,0.49,0.009,0.001))) # FAKE DATA
  output_Age=data.frame(Age=output_Age,RG=rep(c("Early","Late"),each=100))
  prop.test(table(output_Age$Age,output_Age$RG),correct=FALSE)
...
system.time({
  testoutput2 <- scan(myfilenames[[mixname]][1],sep="")
  
  mixsampsize2 <- (length(testoutput2)/length(groupvec))/(floor(nreps[mixname]/thin))
})
...
# This is how we'll do the FL/Weight/K for each RG

# For each iteration
Early_FL = c()
Early_FL_mean = mean(Early_FL)
Early_FL_SE = SD(Early_FL)/sqrt(length(Early_FL))

Early = rnorm(n=1, mean=Early_FL_mean, sd=Early_FL_SE)
# Repeat for Late
...
# This is how we'll do the age proportions for each RG

OUT=array(dim=c(iterations,RGs))
...
g=rgamma(n=4, shape=n+1/length(n), scale=1)
g
p=g/sum(g)
p
